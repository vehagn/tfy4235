...
double intToDouble(int x, double a, double b){
    double c = 0.5*(a+b);
    double d = (b-a)/(((long int)1 << 32));    //can be precomputed given a and b is known
    return c + d*x;
}
double intToDouble2(int x, double a, double b){
    double c = 0.5*(a+b) + 0.5*(b-a)/((1l << 32)-1.0);
    double d = (b-a)/(((long int)1 << 32) - 1.0);    //can be precomputed given a and b is known
    return c + d*x;
}
...
void swapbits(int * x1,int * y1,int a,int b){ //
    int x = *x1;
    int y = *y1;
    // z = z(a,b) = 2^(b+1) - 2^(a), if b < a, z = z(b,a)
...
int main(){
    ...
    for (i = 0; i < Ngen; i++){
        ...
        printf("%d: most fit individual: i=%d -> %f, fitness=%f\n",i,imax,xmax,fmax);
    ...
    printf("%.12f\n",intToDouble(0,-1.0,1.0));
    printf("%.12f\n\n",intToDouble2(0,-1.0,1.0));

    printf("%.12f\n",intToDouble((1<<31)-1,-1.0,1.0));
    printf("%.12f\n\n",intToDouble2((1<<31)-1,-1.0,1.0));

    printf("%.12f\n",intToDouble((1 << 31),-1.0,1.0));
    printf("%.12f\n\n",intToDouble2((1 << 31),-1.0,1.0));
    return 0;
}
